...
export const protocolsFormSchema = z
  .object({
    rigPlc: z.object({
  ...
  .superRefine((data, ctx) => {
    if (data.rigPlc.type === "modbus-tcp") {
      if (
        !data.rigPlc.modbusEndpoints ||
        data.rigPlc.modbusEndpoints.length === 0
      ) {
        ctx.addIssue({
      ...
      if (
        !data.rigPlc.ethernetEndpoint ||
        data.rigPlc.ethernetEndpoint.trim() === ""
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
